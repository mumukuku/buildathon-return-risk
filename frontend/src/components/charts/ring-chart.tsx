...
// Bug fix (upstream bklit-ui): `Ring` is exported as `memo(function Ring(...))`,
// which makes `child.type` an object (`{$$typeof: Symbol(react.memo), type: ...}`),
// not a plain function. The original `typeof child.type === "function"` check
// is therefore always false for a memoized Ring, silently breaking detection
// and falling through to broken layout. We unwrap memo's `.type` before
// checking the inner function's name/displayName.
function unwrapMemoType(type: unknown): unknown {
...
function isRing(child: ReactNode): boolean {
  ...
  const inner = unwrapMemoType(child.type);
  return (
    typeof inner === "function" &&
    ((inner as { displayName?: string }).displayName === "Ring" ||
      (inner as { name?: string }).name === "Ring")
  );
}
...
function isRingCenter(child: ReactNode): boolean {
  ...
  return (
    typeof inner === "function" &&
    ((inner as { displayName?: string }).displayName === "RingCenter" ||
      (inner as { name?: string }).name === "RingCenter")
  );
}
